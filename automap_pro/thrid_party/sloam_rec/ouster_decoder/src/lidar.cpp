...
void LidarScan::DecodeColumn(const uint8_t* const col_buf,
  ...
  for (int ipx = 0; ipx < pf.pixels_per_column; ++ipx) {
    ...
    if (status) {
      ...
      // Extract the RANGE value from the column.
      uint16_t raw_range;
      pf.col_field(px_buf, ouster::sensor::ChanField::RANGE, &raw_range, 1);
      ...
      if (min_range <= range && range <= max_range) {
        ...
        // Extract the SIGNAL value using the new API.
        uint16_t signal_val;
        pf.col_field(px_buf, ouster::sensor::ChanField::SIGNAL, &signal_val, 1);
        s16u = signal_val;
      }
      // Optionally, if you need ambient, you might do:
      // uint16_t ambient_val;
      // pf.col_field(px_buf, ouster::sensor::ChanField::AMBIENT, &ambient_val, 1);
      // s16u += ambient_val;
    }
