...
| 参数 | 默认值 | 建议范围 | 说明与取舍 |
|------|--------|----------|------------|
| **backend.isam2.prior_variance** | **1e-8** | **1e-8～1e-6** | 仅作用于**第一个/固定子图节点**（6 维位姿先验方差）。过小（如 1e-12）≈ 过强约束，可能加重数值刚度、不利于优化器在回环/GPS 下自然收束 → 间接影响对齐与清晰度；过大则首节点易漂移。**默认 1e-8** 平衡“固定原点”与“避免过约束”；若仍觉地图略僵或希望更柔和，可试 **1e-6**。 |

**结论**：全局点云清晰度**主要**由 `map.voxel_size`、`keyframe.retain_cloud_body` 与位姿质量（回环/HBA）决定；先验仅锁首节点，调 `prior_variance` 为次要手段，用于在保证原点稳定的前提下减轻过约束。
